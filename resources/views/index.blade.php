...
@push('js')
    ...
    <script>
        // Data dari server
        const maleCount = {{ $maleCount ?? 0 }};
        const femaleCount = {{ $femaleCount ?? 0 }};
        const topPekerjaanLabels = {!! json_encode($topPekerjaan->pluck('nama')->toArray()) !!};
        const topPekerjaanData = {!! json_encode($topPekerjaan->pluck('pegawai_count')->toArray()) !!};
        ...
        new Chart(ctx2, {
            type: 'bar',
            data: {
                labels: topPekerjaanLabels,
                datasets: [{
                    label: 'Jumlah Pegawai',
                    data: topPekerjaanData,
